...
const validateBarcode = async () => {
  ...
  try {
    const response = await axios.post(
      `/adjustments/${props.product.id}/validate-barcode`,
      {
    ...
    if (error.response && error.response.data && error.response.data.message) {
      ...
      barcodeError.value =
        "Error al validar el código de barras. Inténtalo de nuevo.";
    }
...
const processCount = async () => {
  ...
  if (!props.product || !barcodeMatched.value || countedQuantity.value < 0) {
    countError.value =
      "Por favor, valida el producto y proporciona una cantidad contada válida.";
    return;
  }
...
<template>
  ...
  >
    <VCard>
      ...
      <VCardText>
        <VAlert v-if="barcodeError" type="error" dense dismissible>{{
          barcodeError
        }}</VAlert>
        <VTextField
      ...
      <VCardActions class="p-4">
        ...
        <VBtn color="secondary" variant="outlined" @click="closeModal"
          >Cancelar</VBtn
        >
        <VBtn color="primary" variant="flat" @click="processCount"
