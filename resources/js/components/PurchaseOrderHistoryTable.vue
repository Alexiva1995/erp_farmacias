...
const emit = defineEmits(["update:options", "view"]);
...
const headers = [
  ...
  { title: "% entregado", key: "percentage_arrived", sortable: true },
...
<template>
  <VCard>
    ...
    >
      ...
      <template #item.percentage_arrived="{ item }">
        <span class="font-weight-medium">
          {{ item.percentage_arrived.toLocaleString("es-VE", { minimumFractionDigits: 2 }) }} %
        </span>
      </template>
      ...
      <template #item.actions="{ item }">
        <VTooltip text="Ver Órden de Compra" location="top">
          <template #activator="{ props }">
            <IconBtn v-bind="props" @click="emit('view', item)" color="info">
