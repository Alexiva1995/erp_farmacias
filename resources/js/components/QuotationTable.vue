...
const props = defineProps({
  ...
  dishesLoading: { type: Boolean, default: false },
  dishFilterQuery: { type: String, default: "" },
  selectedDishCategory: { type: [String, null], default: null },
  dishCategories: { type: Array, default: () => [] },
  filteredDishes: { type: Array, default: () => [] },
...
const emit = defineEmits([
  ...
  'failures-products',
  'update:activeTab',
  'update:dishFilterQuery',
  'update:selectedDishCategory',
  'add-dish',
  'add-pack'
]);
...
watch(() => props.products, (newProducts) => {
  ...
  newProducts.forEach(product => {
    let currentQty;
    if (product.item_type === 'dish') {
      currentQty = inputQuantities.value.get(product.id) || 1;
    } else if (product.valid_stock_sum === 0) {
      currentQty = 0;
    } else {
...
const handleInputQuantityChange = (productId, val) => {
  ...
  if (!prod) return;
  const isDish = prod.item_type === 'dish';
  const maxQty = prod.valid_stock_sum ?? 0;
...
const handleAddDish = (dish) => {
  const qty = inputQuantities.value.get(dish.id) || 1;
  emit('add-dish', { dish, quantity: qty });
  inputQuantities.value.set(dish.id, 1);
};
      ...
      >
        ...
        <template #item.name="{ item }">
          <div class="d-flex flex-column py-2" style="max-inline-size: 300px;">
            <div class="d-flex align-center flex-wrap gap-1 mb-0 pb-0">
              <span class="text-subtitle-2 font-weight-black text-uppercase leading-tight" style="font-size: 0.85rem !important; white-space: normal;">
              ...
              <VChip v-if="item.item_type === 'dish'" size="x-small" color="primary" variant="tonal" class="ms-1 font-weight-bold">MENÚ</VChip>
              <VChip v-if="item.item_type === 'pack'" size="x-small" color="secondary" variant="tonal" class="ms-1 font-weight-bold">PACK</VChip>
            </div>
            <div class="text-caption leading-tight d-flex align-center gap-1 mt-1 pt-0">
              ...
              <template v-if="item.item_type !== 'pack'">
                ...
                <span class="text-primary font-weight-bold uppercase" style="font-size: 0.75rem !important;">
                  {{ item.laboratory_name || item.laboratory?.name || 'S/L' }}
                </span>
              </template>
            </div>
          </div>
        ...
        <template #item.add_action_with_quantity="{ item }">
          <div class="d-flex align-center gap-2 justify-center">
            <VTextField
              ...
              type="number"
              min="0"
              :max="item.item_type === 'dish' ? undefined : item.valid_stock_sum"
              density="compact"
              variant="outlined"
              hide-details
              single-line
              class="quantity-input-field font-weight-black"
              :disabled="item.item_type !== 'dish' && item.valid_stock_sum === 0"
            />
            <VBtn
        ...
        >
          <div class="pa-4">
            <div class="d-flex justify-space-between align-start mb-2">
              <span class="font-weight-black text-xs text-primary">#{{ item.id }}</span>
              <VChip
                :color="item.valid_stock_sum > 0 ? 'success' : 'error'"
                size="x-small"
                variant="flat"
            ...
            <div class="d-flex gap-1 mb-3">
              ...
              <VChip v-if="item.item_type === 'dish'" size="x-small" color="primary" variant="flat" class="font-weight-black px-2 shadow-sm">MENÚ</VChip>
              <VChip v-if="item.item_type === 'pack'" size="x-small" color="secondary" variant="flat" class="font-weight-black px-2 shadow-sm">PACK</VChip>
            </div>
            ...
            <div class="d-flex align-center gap-2 mt-2">
              ...
              <VBtn
                ...
                @click="item.item_type === 'product' ? handleAddProduct(item.id) : (item.item_type === 'dish' ? handleAddDish(item) : handleAddProduct(item.id))"
                :disabled="(inputQuantities.get(item.id) ?? 0) <= 0 || (item.item_type !== 'dish' && item.valid_stock_sum === 0)"
              >
