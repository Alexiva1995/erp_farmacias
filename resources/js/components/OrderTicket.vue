  ...
  class="pa-2 text-start">
    ...
    <div class="d-flex justify-space-between align-start textoPrint mb-1">
     ...
     <div class="d-flex justify-space-between align-start textoPrint mb-1">
      ...
      <span class="textoPrint">{{orderData.client.name}} {{orderData.client.last_name}}</span>
    </div>

     <div class="d-flex justify-space-between align-start textoPrint mb-1">
      <span class="textoPrint">Documento:</span>
      <span class="textoPrint">{{orderData.client.identification_type}} {{orderData.client.identification}}</span>
    </div>
      ...
      <div v-for="item in orderProducts" :key="item.id" class="ticket-item">
          ...
          <span class="ticket-item-name">{{ item.title }}</span>
           <span class="ticket-item-total">
            {{ formatCurrency(getItemPriceByCurrency(item, selectedCurrency) * item.selectedQuantity, selectedCurrency) }}
          </span>
      </div>
     <hr />
      <div class="ticket-total d-flex justify-space-between align-center">
          ...
          <span class="text-end font-weight-black tituloAzulPrint"> {{ formatCurrency(totalAmount, selectedCurrency) }}
          </span>
      </div>
