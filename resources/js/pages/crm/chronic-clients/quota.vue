...
const fetchMatrix = async () => {
  ...
  try {
    const response = await $api("/crm/chronic-clients/daily-quotas-matrix", {
      params: {
    ...
    dailyQuota.value = payload.daily_quota || 5;
    ...
    console.error("Error cargando matriz de cuotas de fidelización:", error);
    toast.error("No se pudo cargar el reporte de cuotas de fidelización.");
  } finally {
...
const exportToCsv = () => {
  ...
  link.setAttribute("download", `matriz_cuotas_fidelizacion_${selectedMonth.value}_${selectedYear.value}.csv`);
          ...
          <div class="d-flex align-center gap-3">
            ...
            <div>
              <h1 class="text-h6 font-weight-black text-high-emphasis ma-0 text-uppercase">
                Cumplimiento de Cuotas de Fidelización
              </h1>
              <span class="text-xs text-medium-emphasis">
                Monitoreo del avance y metas diarias de pacientes contactados (Meta: {{ dailyQuota }}/día por operador).
              </span>
            </div>
          </div>
          ...
          <VCardText class="pa-4 d-flex align-center gap-3">
            ...
            <div>
              ...
              <div class="text-h6 font-weight-black text-high-emphasis">
                {{ summary.total_month_counts }} <span class="text-xs text-medium-emphasis font-weight-regular">contactos</span>
              </div>
            </div>
          </VCardText>
        </VCard>
        ...
        <VCard class="rounded-lg border shadow-sm bg-surface">
          <VCardText class="pa-4 d-flex align-center gap-3">
            ...
            <div>
              ...
              <div class="text-h6 font-weight-black text-high-emphasis">
                {{ summary.daily_average }} <span class="text-xs text-medium-emphasis font-weight-regular">p/día</span>
              </div>
            </div>
          </VCardText>
        </VCard>
        ...
        <VCard class="rounded-lg border shadow-sm bg-surface">
          <VCardText class="pa-4 d-flex align-center gap-3">
            ...
            <div class="overflow-hidden">
              <span class="text-xs text-medium-emphasis font-weight-bold uppercase">Top Fidelizador</span>
    ...
    <VCard variant="outlined" class="rounded-lg border shadow-sm bg-surface overflow-hidden">
      ...
      <div class="d-flex align-center justify-space-between flex-wrap gap-3 pa-3 bg-var-theme-background border-b">
        ...
        <div class="d-flex align-center gap-3 flex-wrap text-xs text-medium-emphasis">
          ...
          <div class="d-flex align-center gap-1">
            ...
            <span>En progreso (&lt;{{ dailyQuota }})</span>
          </div>
          <div class="d-flex align-center gap-1">
      ...
      <div v-else-if="rows.length === 0" class="text-center py-12 text-disabled">
        ...
        <p class="text-xs text-medium-emphasis">
          No se encontraron contactos de fidelización registrados para el período seleccionado.
        </p>
        <VBtn
          variant="tonal"
          color="primary"
                ...
                >
                  ...
                  <div class="d-flex flex-column flex-grow-1 min-width-0">
                    ...
                    <span
                      ...
                      :class="getEmployeeCell(row, emp).fulfilled ? 'text-success' : 'text-primary'"
                    >
                      {{ getEmployeeCell(row, emp).count }}/{{ getEmployeeCell(row, emp).quota }}
                    </span>
                  </div>
