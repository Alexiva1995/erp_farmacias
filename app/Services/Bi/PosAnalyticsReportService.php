    ...
    public function getPosDashboard(array $filters): array
    {
        ...
        $dailySeries = [
            'series' => [
                [
                    'name' => 'Promedio Venta',
                    'data' => collect($temporal['daily_focus'])->pluck('avg_amount')->map(fn($v) => round($v, 2))->toArray()
                ]
            ],
            'categories' => collect($temporal['daily_focus'])->pluck('day_name')->toArray()
        ];

        // Calcular porcentajes por franja horaria
            ...
            'series' => [
                [
                    ...
                    'data' => collect($temporal['hourly_slots'])->map(function($slot) use ($totalHourlyCount) {
                        return [
                            'x' => str_pad($slot->hour, 2, '0', STR_PAD_LEFT) . ':00',
