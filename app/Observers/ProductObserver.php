...
class ProductObserver
{
    ...
    public static function handleInvoiceMovement(Invoice $invoice): void
    {
        foreach ($invoice->details as $detail) {
            self::handleInvoiceDetailMovement($invoice, $detail);
        }
    }

    /**
     * Manejar movimiento de inventario para un renglón individual de factura.
     */
    public static function handleInvoiceDetailMovement(Invoice $invoice, InvoiceDetail $detail, ?int $productLotId = null): void
    {
        // Obtener la instancia fresca del producto desde la base de datos
        ...
        $rate = $invoice->exchange_rate > 0 ? (float) $invoice->exchange_rate : 1.0;
        $incomingUnitCost = (float) $detail->unit_cost;
        ...
        $totalValueIncoming = (float) $detail->quantity * $incomingUnitCost;
        $newUnitCost = 0.0;
        ...
        // Buscar el lote asociado recién creado si no fue provisto
        if (!$productLotId) {
