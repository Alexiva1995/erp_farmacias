...
class GenerateDailyFiscalZReportCommand extends Command
{
    ...
    protected $signature = 'fiscal:generate-z-reports 
                            ...
                            {--start-number=740 : Número de inicio si la tabla está vacía}
                            {--force : Sobrescribir reportes existentes para la fecha}';
     ...
     * @var string
     */
    protected $description = 'Genera los reportes Z diarios fiscales a partir del historial de facturación.';
    ...
    public function handle(FiscalZReportService $zReportService): int
    {
        ...
        $startNumber = (int) $this->option('start-number');

        if ($specificDate) {
            $this->info("Generando Reporte Z para la fecha: {$specificDate}...");
            $report = $zReportService->generateForDate($specificDate, $force);
        ...
        if ($this->hasOption('month') && $monthOption === null && !$specificDate && !$force && !app()->runningInConsole()) {
            ...
            $report = $zReportService->generateForDate($yesterday, true);
        ...
        $this->info("Generando reportes Z para {$year}-" . str_pad((string)$month, 2, '0', STR_PAD_LEFT) . " (Inicio base: #{$startNumber})...");

        $reports = $zReportService->generateForMonth($year, $month, $startNumber);
